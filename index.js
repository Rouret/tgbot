...
const API_KEY_JOKE = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiMjU5ODE3MzI4NDU4MzM0MjExIiwibGltaXQiOjEwMCwia2V5IjoiT0cxbFJ6U05SWTVIOVNGWnN6NWJkQ2Z0OUU3QVphbVFMZHc5cG5NQ0xpUUNzVmhOanEiLCJjcmVhdGVkX2F0IjoiMjAyMC0xMC0yMlQxOTo0NTozMSswMjowMCIsImlhdCI6MTYwMzM4ODczMX0.HplPyggkIIFOTgnisivNaf75825jQv7O2063Msa_0c0"
const phraseList = [
    "tg",
    "TG",
    "Ferme la",
...
client.on('message', msg => {
    ...
    if (
        ...
        message_content.exec == "!" &&
        message_content.command == "joke"
    ) {
        TG.joke(msg)
    } else if (
        msg.content.match(regex)
    ) {
        TG.triso(msg);
    } else if (
        message_content.exec == "!" &&
...
class tg {
    tg(msg) {
        msg.delete();
        msg.channel.send(phraseList[lib.getRandom(0, phraseList.length - 1)] + " " + message_content.args[0]);
    ...
    ping(msg, message_content) {
        ...
        client.users.cache.get(person).send(message_content.args.join().replace(/,/g, " "));
        msg.channel.send("Ouallala un message privée a été envoyé," + trooooll[lib.getRandom(0, trooooll.length - 1)])
    }
    joke(msg) {
        msg.delete();
